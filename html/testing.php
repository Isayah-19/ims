<?php
	if (isset($_GET['row']) && isset($_GET['action'])) {
        ...
        $message = "";
        if ($stat == "deactivate") {
            $query = "update couns_appointmentype set appmnstat = 'Inactive' where appmntype = '$key'";
            $result = mysqli_query($db,$query);
            if ($result) {
                $message = "'.$key.' is now deactivated!"; 
            }
        } else {
            $query = "update couns_appointmentype set appmnstat = 'Active' where appmntype = '$key'";
            $result = mysqli_query($db,$query);
            if ($result) {
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered">
    	...
    	<tbody id="visitlist">
    		<?php
	    		...
	    		include 'config.php';
	    		$query = 'select * from couns_appointmentype';
	    		$result = mysqli_query($db,$query);
	    		if ($result) {
	    			while ($row = $result -> fetch_assoc()) {
		    			echo '
				    		<tr>
				    			<td id="visit-type">'.$row["appmntype"].'</td>
				    			<td>description</td>
				    			<td id="visit-stat">'.$row['appmnstat'].'</td>
				    			<td>
